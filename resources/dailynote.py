...
class DailyNoteAddResource(Resource) :
    ...
    def post(self,childId):
        ...
        try:
            ...
            query = '''insert into dailyNote
                    (childId,title,contents,photoUrl,dailyTemperCheck,dailyMealCheck,dailyNapCheck,dailyPooCheck)
                    values
                    (%s,%s,%s,%s,%s,%s,%s,%s);'''
            #2-3. 쿼리에 매칭되는 변수 처리! 중요! 튜플로 처리해준다!(튜프은 데이터변경이 안되니까?)
            # 위의 %s부분을 만들어주는거다
            record = (childId,title,contents,file_url,dailyTemperCheck,dailyMealCheck,dailyNapCheck,dailyPooCheck)
            #2-4 커서를 가져온다
